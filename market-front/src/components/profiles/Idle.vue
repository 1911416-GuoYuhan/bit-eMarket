    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="handleIdleStatus(scope.$index, scope.row)" style="background-color: #266720;font-size: 16px;font-weight: bold;color: white">{{scope.row.status ? '下架' : '上架' }}</el-button>
          <el-button
            size="mini"
            type="danger"
            @click="handleIdleDelete(scope.$index, scope.row)" style="background-color: #ba2407;font-size: 16px;font-weight: bold;color: white">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
