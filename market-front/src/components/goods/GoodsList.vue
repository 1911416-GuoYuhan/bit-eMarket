<template>
    ...
    <el-row>
      <el-col
        ...
        <div class="goodsItem"  @click="goContent(item.id, item.userId)" style="margin-bottom: 20px">
          <el-card class="goods-item">
            ...
            <span style="font-size: 20px;font-weight: bold">{{item.name}}</span>
              <div class="bottom clearfix">
                <time class="time" v-model="goodsArray[index].endTime">
                  下架时间：{{ item.endTime }}
                </time>
                <el-button type="text" class="button" v-model="goodsArray[index].price" style="font-weight: bold">
                  ￥{{item.price}}
    ...
    <el-pagination
      ...
      style="margin-top: 15px;background-color: #ba2407"
      align="center"
      background
      ...
      InitPageTotal(){
        ...
        Axios.get(Api.getGoodsCount(),{
          ...
          .then(function (response) {
            console.log('getGoodsCount',response)
